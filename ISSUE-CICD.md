...
Status: **Produktiv bestätigt.** Der manuell gestartete Self-Hosting-Lauf `29993667180` auf `main` installierte Drydock `1.20264.12396` aus PSGallery und fand `Publish-PowerShellModuleRelease` über den neuen Command-Guard. Anschließend veröffentlichte die Funktion Version `1.20264.12416` erfolgreich und getrennt an die Targets `Local`, `GitHubPackages` und `PSGallery`. Das Log bestätigt für jedes Ziel die Abschlussmeldung aus `Eigenverft.Manifested.Drydock.Publishing.ps1`; der GitHub-Token wurde bei der NuGet-Registrierung maskiert und die temporären Registrierungen wurden bereinigt. Der Lauf erzeugte den automatischen Versionscommit `4366ca1`, den Tag `v1.20264.12416` und den veröffentlichten GitHub-Release `v1.20264.12416`. Damit ist der Bootstrap-Pfad vollständig durch die neue Drydock-Abstraktion ersetzt.
