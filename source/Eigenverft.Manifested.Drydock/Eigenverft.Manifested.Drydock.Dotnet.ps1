function Enable-TempDotnetTools_OldWorking {
<#
.SYNOPSIS
...
#>
    ...
    $mf = Resolve-Path -LiteralPath $ManifestFile -ErrorAction Stop
    _Write-StandardMessage -Message ("[SOURCE] [dotnet-tools] Manifest:        {0}" -f $mf.Path) -Level 'INF'
    ...
    New-Item -ItemType Directory -Force -Path $ToolPath | Out-Null
    _Write-StandardMessage -Message ("[CACHE] [dotnet-tools] Cache (toolpath): {0}" -f $ToolPath) -Level 'INF'
    ...
    $before = _GetToolsInPath -Path $ToolPath
    if ($before.Count -gt 0) {
        _Write-StandardMessage -Message ("[CACHE] [dotnet-tools] Cache existing:  {0}" -f $before.Count) -Level 'INF'
        foreach ($k in ($before.Keys | Sort-Object)) {
            _Write-StandardMessage -Message ("[CACHE]   - {0} {1}" -f $k, $before[$k]) -Level 'DBG'
        }
    } else {
        _Write-StandardMessage -Message "[CACHE] [dotnet-tools] Cache existing:  none" -Level 'INF'
    }
    ...
    foreach ($prop in $toolProps) {
        ...
        $status = "AlreadyPresent"
        if (-not $unchanged) {
            _Write-StandardMessage -Message ("[CHECK] [dotnet-tools] Ensuring:       {0}@{1}" -f $id, $ver) -Level 'INF'
            $ok = _EnsureExactTool -Path $ToolPath -Id $id -Version $ver -NoCache:$NoCache -TryUpdateFirst:$present
            if (-not $ok) { throw "Failed to ensure $id@$ver in $ToolPath." }
            if ($present) {
                $status = "Updated"
    ...
    # -----------------------
    _PrependPathIfMissing -Path $ToolPath
    _Write-StandardMessage -Message ("[PATH] [dotnet-tools] PATH updated:    {0}" -f $ToolPath) -Level 'INF'
    ...
    for ($i = 0; $i -lt $toolsResult.Count; $i++) {
        ...
        if ($after.ContainsKey($rid)) {
            if ($toolsResult[$i].Version -ne $after[$rid]) {
                _Write-StandardMessage -Message ("[VALUE] [dotnet-tools] Resolved:       {0} -> {1}" -f $toolsResult[$i].Version, $after[$rid]) -Level 'INF'
            }
            $toolsResult[$i].Version = $after[$rid]
        }
    ...
    _Write-StandardMessage -Message ("[SUMMARY] [dotnet-tools] Commands (manifest): {0} tool(s)" -f $rows.Count) -Level 'INF'
