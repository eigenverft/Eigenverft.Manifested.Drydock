...
function Invoke-Exec2 {
    <#
        ...
        Return shaping (applies only when -CaptureOutput is $true):
        - ReturnAsArray        : Ensures an object[] (default).
        - ReturnStringAsArray  : Converts each item to [string] and returns string[].
        - ReturnAsText         : Returns a single [string] with all (stringified) lines joined by the platform newline.
        ...
        Shapes the return value when output is captured (ignored if -CaptureOutput:$false). Allowed:
        - ReturnAsArray (default): Ensures object[] via @($result).
        - ReturnStringAsArray   : Casts each item to [string] and returns string[].
        - ReturnAsText          : Joins (stringified) output into a single [string] with newline separators.

        .OUTPUTS
        System.Object[]  (when -CaptureOutput and -ReturnType ReturnAsArray)
        System.String[]  (when -CaptureOutput and -ReturnType ReturnStringAsArray)
        System.String    (when -CaptureOutput and -ReturnType ReturnAsText)
        System.Object    (returns $null when -CaptureOutput:$false)
    ...
    param(
        ...
        [Alias('rt')]
        [ValidateSet('ReturnAsArray','ReturnStringAsArray','ReturnAsText')]
        [string]$ReturnType = 'ReturnAsText'
    )
    ...
    # Reviewer note: Return shaping logic only applies when output is captured; otherwise $null is returned.
    if (-not $CaptureOutput) { return $null }

    switch ($ReturnType.ToLowerInvariant()) {
        'returnasarray' {
            if ($null -eq $result) { return @() }
            return @($result)          # ensure array (object[])
        }
        'returnstringasarray' {
            if ($null -eq $result) { return @() }
            return @($result | ForEach-Object { [string]$_ })  # string[]
        }
        'returnastext' {
            if ($null -eq $result) { return '' }
            $lines = $result | ForEach-Object { [string]$_ }
            return ($lines -join [Environment]::NewLine)       # single string
        }
        default {
            if ($null -eq $result) { return @() }
            return @($result)
