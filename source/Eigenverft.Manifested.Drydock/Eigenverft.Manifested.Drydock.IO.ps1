...
#>
    ...
    function local:_Copy-ItemWithRetry {
        ...
        if ($skipCopy) {
            _Write-StandardMessage -Message "[STATUS] Skipping copy; destination already satisfies comparison policy '$CopyComparisonPolicy' for '$fileName'."
            return
        }
    ...
    try {
        ...
        _Write-StandardMessage -Message "[SUMMARY] Sync-DirectoryTreeLockSafe completed. Created $createdDirectoriesCount directories, removed $removedDirectoriesCount directories, removed $removedFilesCount files, processed $copiedFilesCount files (copied or skipped based on policy)."
    }
    finally {
...
function Sync-DirectoryTreeLockSafe2 {
<#
.SYNOPSIS
Mirrors a source directory tree to a destination directory with retries and locking.
...
#>
    ...
    try {
        ...
        _Write-SummaryMessage -Message "[SUMMARY] Sync-DirectoryTreeLockSafe completed. Created $createdDirectoriesCount directories, removed $removedDirectoriesCount directories, removed $removedFilesCount files, processed $copiedFilesCount files (copied or skipped based on policy)."
    }
    finally {
        if ($destinationLockAcquired -and ($null -ne $destinationRootResolved)) {
            _Release-Lock -RootPath $destinationRootResolved -LockFileName $destinationLockFileName
