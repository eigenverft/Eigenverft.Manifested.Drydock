...
{
    public static class SafeCrc32
    {
        /// <summary>
        /// Computes a CRC32 checksum for the specified file path.
        /// Returns a lower-case 8-character hex string on success, or null on any failure.
        /// This method MUST NOT throw for normal operational errors; it is intended
        /// as a best-effort helper for PowerShell scripts.
        /// </summary>
        public static string ComputeFileCrc32OrNull(string filePath)
        {
            ...
            {
                // Any IO / access / sharing / unexpected error just yields null.
                return null;
            }
        }

        // --- Internal CRC32 implementation ---
...
if (-not ("Eigenverft.Tools.SafeCrc32" -as [type])) {
    if (Test-Path Variable:crc32HelperSource) {
        if ($crc32HelperSource) {
            try {
                Add-Type -TypeDefinition $crc32HelperSource -Language CSharp -ErrorAction Stop | Out-Null
            }
            catch {
                Write-Warning "SafeCrc32: Add-Type failed, continuing without C# helper. $($_.Exception.Message)"
            }
        }
        else {
            Write-Warning "SafeCrc32: variable 'crc32HelperSource' is empty, continuing without C# helper."
        }
    }
    else {
        Write-Warning "SafeCrc32: variable 'crc32HelperSource' not found, continuing without C# helper."
    }
}
...
#>
    ...
    try {
        ...
        foreach ($file in $sourceFiles) {
            ...
            _Copy-ItemWithRetry -SourceFullPath $file.FullName -DestinationFullPath $destFileFullPath -RetryCount $CopyRetryCount -RetryDelaySeconds $CopyRetryDelaySeconds
...
Sync-DirectoryTreeLockSafe -SourcePath "C:\Artificial_Intelligence\ibm-granite\granite-4.0-micro" -DestinationPath "E:\temp\granite-mirror"