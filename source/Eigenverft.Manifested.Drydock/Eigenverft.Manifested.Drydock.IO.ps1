...
#>
    ...
    if (-not (Test-Path -LiteralPath $Path -PathType Container)) {
        throw "[CHECK] The specified path '$Path' does not exist or is not a directory."
    }
    ...
    if (-not $pat -or $pat.Count -eq 0) {
        throw "[CHECK] Pattern must not be empty."
    }

    # Prepare traversal using explicit stack; avoid -Recurse.
    $rootItem = $null
    try { $rootItem = Get-Item -LiteralPath $Path -ErrorAction Stop }
    catch { throw "[CHECK] Path '$Path' is not accessible: $_" }
    ...
    while ($stack.Count -gt 0) {
        ...
        try {
            ...
            local:_Write-StandardMessage -Message ("[SKIP] Cannot list directories in '{0}': {1}" -f $dir.FullName, $_) -Level 'WRN'
        }
        foreach ($sd in $subs) { $stack.Push($sd) }
        ...
        try {
            ...
            local:_Write-StandardMessage -Message ("[SKIP] Cannot list files in '{0}': {1}" -f $dir.FullName, $_) -Level 'WRN'
        }
    ...
    foreach ($fi in $candidates) {
        try {
            ...
            local:_Write-StandardMessage -Message ("[CHECK] Failed to delete '{0}': {1}" -f $fi.FullName, $_) -Level 'WRN'
        }
    }
    ...
    local:_Write-StandardMessage -Message ("[SUMMARY] Deleted files: {0}" -f $deleted) -Level 'INF'
    if ($RemoveEmptyDirs -eq 'Yes') {
        local:_Write-StandardMessage -Message ("[SUMMARY] Removed empty directories: {0}" -f $removedDirs) -Level 'INF'
    }
}
...
.NOTES
    ...
    Logging: Emits tagged console messages via local:_Write-StandardMessage when creating or validating the target path.
#>
    [CmdletBinding()]
    [OutputType([string])]
    param(
    ...
    if ([System.IO.File]::Exists($fullPath)) {
        local:_Write-StandardMessage -Level 'ERR' -Message ("[CHECK] A file already exists at '{0}'; cannot create a directory at this path." -f $fullPath)
    ...
    try {
        ...
        local:_Write-StandardMessage -Level 'ERR' -Message ("[CREATE] Failed to create or access directory '{0}': {1}" -f $fullPath, $_)
        throw ("Failed to create or access directory '{0}': {1}" -f $fullPath, $_)
    }

    if (-not $existed) {
        local:_Write-StandardMessage -Message ("[CREATE] Created directory: {0}" -f $fullPath)
    }
...
- Preserves source CreationTimeUtc and LastWriteTimeUtc on destination files
...
Controls when an existing destination file is considered identical to the source and the copy
operation can be skipped. Default is 'LengthAndLastWriteTimeUtc'.

Valid values:
- 'None'                      : Always copy the file (no pre-check).
...
#>
    ...
    # Wrapper: detail-level messages (suppressed when LogDetailLevel='Summary')
    function local:_Write-DetailMessage {
        ...
        if ($LogDetailLevel -eq 'Summary') {
            return
        }

        _Write-StandardMessage @PSBoundParameters
    }

    # Wrapper: summary-level messages (always shown; still subject to severity MinLevel)
    function local:_Write-SummaryMessage {
        [Diagnostics.CodeAnalysis.SuppressMessage("PSUseApprovedVerbs","")]
        [CmdletBinding()]
        param(
            [Parameter(Mandatory = $true)][AllowEmptyString()][string]$Message,
    ...
    function local:_Get-DirectoryTraversalOrder {
        ...
        _Write-DetailMessage -Message "[SUMMARY] Directory traversal order computed; found $($ordered.Count) items (including lock files)."
    ...
    function local:_Remove-StaleLockIfExpired {
        ...
        try {
            ...
            _Write-DetailMessage -Message "[WRN] Removed stale lock '$LockFilePath'."
            return $true
        }
        catch {
            $ex = $_.Exception
    ...
    function local:_Acquire-SourceLock {
        ...
        while ($true) {
            ...
            $attempt = $attempt + 1
            _Write-DetailMessage -Message "[CHECK] Writer lock '$writerLockPath' in source detected, waiting $RetryDelaySeconds seconds (attempt $attempt of $RetryCount)."
        ...
        try {
            ...
            _Write-DetailMessage -Message "[STATUS] Acquired source lock '$lockFilePath'."
        ...
        while ($true) {
            ...
            if (-not $writerExists -and -not $readerExists) {
                try {
                    New-Item -ItemType File -Path $lockFilePath -Force -ErrorAction Stop | Out-Null
                    _Write-LockTokenFile -LockFilePath $lockFilePath -LockTokenExpiryHours $LockTokenExpiryHours
                    _Write-DetailMessage -Message "[STATUS] Acquired lock '$lockFilePath'."
                    break
                }
                catch {
                ...
                $attempt = $attempt + 1
                _Write-DetailMessage -Message "[CHECK] Destination '$RootPath' locked (writer: $writerExists, readers: $readerExists). Waiting $RetryDelaySeconds seconds (attempt $attempt of $RetryCount)."
                Start-Sleep -Seconds $RetryDelaySeconds
            }
        }
    }
    ...
    function local:_Release-Lock {
        ...
        if (Test-Path -LiteralPath $lockFilePath -PathType Leaf) {
            try {
                Remove-Item -LiteralPath $lockFilePath -Force -ErrorAction Stop
                _Write-DetailMessage -Message "[STATUS] Released lock '$lockFilePath'."
            }
            catch {
            ...
            _Write-DetailMessage -Message "[CHECK] Lock file '$lockFilePath' already absent on release."
        }
    }

    # copies a single file with retry logic, preserves timestamps, and can skip based on comparison policy
    function local:_Copy-ItemWithRetry {
        ...
        # optional pre-copy comparison: decide if we can skip the copy
        $destExists = Test-Path -LiteralPath $DestinationFullPath -PathType Leaf
        $skipCopy = $false

        if ($destExists -and ($CopyComparisonPolicy -ne 'None')) {
            try {
                $sourceInfo = Get-Item -LiteralPath $SourceFullPath -Force
                $destInfo = Get-Item -LiteralPath $DestinationFullPath -Force

                if ($CopyComparisonPolicy -eq 'Length') {
                    if ([long]$sourceInfo.Length -eq [long]$destInfo.Length) {
                        $skipCopy = $true
                    }
                }
                elseif ($CopyComparisonPolicy -eq 'LengthAndLastWriteTimeUtc') {
                    if (([long]$sourceInfo.Length -eq [long]$destInfo.Length) -and ($sourceInfo.LastWriteTimeUtc -eq $destInfo.LastWriteTimeUtc)) {
                        $skipCopy = $true
                    }
                }
                elseif ($CopyComparisonPolicy -eq 'Sha256Hash' -or $CopyComparisonPolicy -eq 'Md5FastHash') {
                    $sourceStream = $null
                    $destStream = $null
                    $hashAlgorithm = $null
                    try {
                        $sourceStream = [System.IO.File]::Open($SourceFullPath, [System.IO.FileMode]::Open, [System.IO.FileAccess]::Read, [System.IO.FileShare]::Read)
                        $destStream = [System.IO.File]::Open($DestinationFullPath, [System.IO.FileMode]::Open, [System.IO.FileAccess]::Read, [System.IO.FileShare]::Read)

                        if ($CopyComparisonPolicy -eq 'Sha256Hash') {
                        ...
                        $sourceHashBytes = $hashAlgorithm.ComputeHash($sourceStream)
                        $destHashBytes = $hashAlgorithm.ComputeHash($destStream)

                        $sourceHash = -join ($sourceHashBytes | ForEach-Object { $_.ToString('x2') })
                        $destHash = -join ($destHashBytes | ForEach-Object { $_.ToString('x2') })
                ...
                _Write-StandardMessage -Message "[WRN] Unable to evaluate copy comparison policy '$CopyComparisonPolicy' for '$SourceFullPath' and '$DestinationFullPath'. Proceeding with copy. $($_.Exception.Message)" -Level 'WRN'
            }
        }

        if ($skipCopy) {
            _Write-DetailMessage -Message "[STATUS] Skipping copy; destination already satisfies comparison policy '$CopyComparisonPolicy' for '$fileName'."
            return
        }
        ...
        while ($attempt -lt $maxAttempts) {
            try {
                ...
                if ($attempt -eq 0 -and $shouldLogLargeFile) {
                    ...
                    _Write-DetailMessage -Message "[STATUS] Currently copying large file (about $sizeMB MB), this may take a while: '$fileName'."
                }

                Copy-Item -LiteralPath $SourceFullPath -Destination $DestinationFullPath -Force -ErrorAction Stop

                try {
                    $sourceInfoAfterCopy = Get-Item -LiteralPath $SourceFullPath -Force
                    $destInfoAfterCopy = Get-Item -LiteralPath $DestinationFullPath -Force
                    $destInfoAfterCopy.LastWriteTimeUtc = $sourceInfoAfterCopy.LastWriteTimeUtc
                    $destInfoAfterCopy.CreationTimeUtc = $sourceInfoAfterCopy.CreationTimeUtc
                }
                catch {
                ...
                if ($attempt -gt 0) {
                    _Write-DetailMessage -Message "[STATUS] Successfully copied '$SourceFullPath' to '$DestinationFullPath' after retry attempt $attempt."
                }
    ...
    _Write-DetailMessage -Message "--- Sync-DirectoryTreeLockSafe: synchronize directory trees ---"
    ...
    try {
        ...
        if (-not (Test-Path -LiteralPath $DestinationPath -PathType Container)) {
            try {
                ...
                _Write-DetailMessage -Message "[CREATE] Created destination root '$DestinationPath'."
            }
            catch {
        ...
        if ($destDirSet.Count -gt 0 -or $destFileSet.Count -gt 0) {
            ...
            foreach ($destItem in $destTraversal) {
                ...
                if ($destItem.PSIsContainer) {
                    if (-not $sourceDirSet.Contains($relativePath)) {
                        try {
                            _Write-DetailMessage -Message "[REMOVE] Removing extra directory '$($destItem.FullName)'."
                    ...
                    if (-not $sourceFileSet.Contains($relativePath)) {
                        try {
                            _Write-DetailMessage -Message "[REMOVE] Removing extra file '$($destItem.FullName)'."
                ...
                if (-not [string]::IsNullOrEmpty($relativePath)) {
                    ...
                    if (-not (Test-Path -LiteralPath $destDirFullPath -PathType Container)) {
                        try {
                            ...
                            $createdDirectoriesCount = $createdDirectoriesCount + 1
                            _Write-DetailMessage -Message "[CREATE] Ensured directory '$destDirFullPath' exists."
                        }
                        catch {
        ...
        foreach ($file in $sourceFiles) {
            ...
            _Copy-ItemWithRetry -SourceFullPath $file.FullName -DestinationFullPath $destFileFullPath -RetryCount $CopyRetryCount -RetryDelaySeconds $CopyRetryDelaySeconds -CopyComparisonPolicy $CopyComparisonPolicy
            $copiedFilesCount = $copiedFilesCount + 1
        }

        _Write-SummaryMessage -Message "[SUMMARY] Sync-DirectoryTreeLockSafe completed. Created $createdDirectoriesCount directories, removed $removedDirectoriesCount directories, removed $removedFilesCount files, processed $copiedFilesCount files (copied or skipped based on policy). From: $SourcePath To: $DestinationPath"
    }
    finally {
