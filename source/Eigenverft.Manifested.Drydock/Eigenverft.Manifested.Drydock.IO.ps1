...
function Copy-FilesRecursively2 {
    ...
    .EXAMPLE
        ...
        Copy-FilesRecursively2 -SourceDirectory "C:\Source" `
                               ...
                               -CopyEmptyDirs $false `
                               -ForceOverwrite $true `
                               -CleanDestination "Root"

    .EXAMPLE
        # Copy all files and clean extra files across the entire destination tree.
        Copy-FilesRecursively2 -SourceDirectory "C:\Source" `
                               -DestinationDirectory "C:\Dest" `
...
function Copy-FilesRecursively3 {
    <#
    .SYNOPSIS
        Recursively copies files from a source directory to a destination directory.
    ...
    .EXAMPLE
        ...
        Copy-FilesRecursively2 -SourceDirectory "C:\Source" `
                               ...
                               -CopyEmptyDirs $false `
                               -ForceOverwrite $true `
                               -CleanDestination "RootFiles"

    .EXAMPLE
        # Copy all files and clean extra files across the entire destination tree (files only).
        Copy-FilesRecursively2 -SourceDirectory "C:\Source" `
                               -DestinationDirectory "C:\Dest" `
                               -CleanDestination "FilesRecursive"

    .EXAMPLE
        # Mirror-style cleanup: remove all extra files and directories in destination.
        Copy-FilesRecursively2 -SourceDirectory "C:\Source" `
                               -DestinationDirectory "C:\Dest" `
                               -CleanDestination "MirrorTree"
    ...
    # Validate that the source directory exists.
    if (-not (Test-Path -Path $SourceDirectory -PathType Container)) {
        _Write-StandardMessage -Message "Source directory '$SourceDirectory' does not exist." -Level 'ERR'
        return
    }
    ...
    if (-not $CopyEmptyDirs) {
        $matchingFiles = Get-ChildItem -Path $SourceDirectory -Recurse -File -Filter $Filter -ErrorAction SilentlyContinue
        if (-not $matchingFiles -or $matchingFiles.Count -eq 0) {
            _Write-StandardMessage -Message "No files matching filter found in source. Skipping directory creation as CopyEmptyDirs is false." -Level 'DBG'
            return
        }
    }
    ...
    switch ($CleanDestination) {
        'RootFiles' {
            _Write-StandardMessage -Message "Cleaning destination root: removing extra files not present in source." -Level 'DBG'
            $destRootFiles = Get-ChildItem -Path $DestinationDirectory -File -Filter $Filter
            foreach ($destFile in $destRootFiles) {
                $sourceFilePath = Join-Path -Path $SourceDirectory -ChildPath $destFile.Name
                if (-not (Test-Path -Path $sourceFilePath -PathType Leaf)) {
                    _Write-StandardMessage -Message "Removing file: $($destFile.FullName)" -Level 'DBG'
                    Remove-Item -Path $destFile.FullName -Force
                }
            }
        }
        'FilesRecursive' {
            _Write-StandardMessage -Message "Cleaning destination tree: removing extra files not present in source (recursive)." -Level 'DBG'
            $destFiles = Get-ChildItem -Path $destFullPath -Recurse -File -Filter $Filter
            foreach ($destFile in $destFiles) {
                $relative = $destFile.FullName.Substring($destFullPath.Length).TrimStart('\')
                $sourceFilePath = Join-Path -Path $sourceFullPath -ChildPath $relative
        ...
        'MirrorTree' {
            _Write-StandardMessage -Message "Full clean: removing extra files and directories to mirror source." -Level 'DBG'
            ...
            foreach ($destDir in $destDirs) {
                ...
                $sourceDirPath = Join-Path -Path $sourceFullPath -ChildPath $relativeDir
                if (-not (Test-Path -Path $sourceDirPath -PathType Container)) {
                    _Write-StandardMessage -Message "Removing directory: $($destDir.FullName)" -Level 'DBG'
                    Remove-Item -Path $destDir.FullName -Recurse -Force
                }
            }
        }
    ...
    if ($CopyEmptyDirs) {
        _Write-StandardMessage -Message "Recreating complete directory structure from source." -Level 'DBG'
        Get-ChildItem -Path $sourceFullPath -Recurse -Directory | ForEach-Object {
            $relativePath = $_.FullName.Substring($sourceFullPath.Length)
            $newDestDir   = Join-Path -Path $destFullPath -ChildPath $relativePath
            if (-not (Test-Path -Path $newDestDir)) {
        ...
        _Write-StandardMessage -Message "Creating directories only for files matching the filter." -Level 'DBG'
        foreach ($file in $matchingFiles) {
            $sourceDir   = Split-Path -Path $file.FullName -Parent
            $relativeDir = $sourceDir.Substring($sourceFullPath.Length)
            $newDestDir  = Join-Path -Path $destFullPath -ChildPath $relativeDir
    ...
    _Write-StandardMessage -Message "Copying files from source to destination." -Level 'DBG'
    if ($CopyEmptyDirs) {
        $filesToCopy = Get-ChildItem -Path $SourceDirectory -Recurse -File -Filter $Filter
    }
    else {
    ...
    foreach ($file in $filesToCopy) {
        ...
        if (-not $ForceOverwrite -and (Test-Path -Path $destFile)) {
            _Write-StandardMessage -Message "Skipping existing file (overwrite disabled): $destFile" -Level 'DBG'
            continue
        }
        ...
        _Write-StandardMessage -Message "Copying file: $($file.FullName) to $destFile" -Level 'DBG'
        if ($ForceOverwrite) {
            Copy-Item -Path $file.FullName -Destination $destFile -Force
        }
        else {
...
function Copy-FilesRecursively4 {
    <#
    .SYNOPSIS
        Recursively copies files from a source directory to a destination directory.
    ...
    .EXAMPLE
        # Copy all *.txt files, create only directories that hold matching files, and clean extra files in the destination root.
        Copy-FilesRecursively4 -SourceDirectory "C:\Source" -DestinationDirectory "C:\Dest" -Filter "*.txt" -CopyEmptyDirs $false -ForceOverwrite $true -CleanDestination "RootFiles"

    .EXAMPLE
        # Copy all files and clean extra files across the entire destination tree (files only).
        Copy-FilesRecursively4 -SourceDirectory "C:\Source" -DestinationDirectory "C:\Dest" -CleanDestination "FilesRecursive"

    .EXAMPLE
        # Mirror-style cleanup: remove all extra files and directories in destination.
        Copy-FilesRecursively4 -SourceDirectory "C:\Source" -DestinationDirectory "C:\Dest" -CleanDestination "MirrorTree"

    .EXAMPLE
        # Copy all files, recreate the full directory tree without cleaning extra files.
        Copy-FilesRecursively4 -SourceDirectory "C:\Source" -DestinationDirectory "C:\Dest"
    #>
    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $true)]
    ...
    _Write-StandardMessage -Message "--- Copy-FilesRecursively4: Recursively copy files and optionally clean destination ---" -Level 'INF'
    ...
    # Validate that the source directory exists.
    if (-not (Test-Path -Path $SourceDirectory -PathType Container)) {
        _Write-StandardMessage -Message "[ERROR] Source directory '$SourceDirectory' does not exist." -Level 'ERR'
        return
    }
    ...
    if (-not $CopyEmptyDirs) {
        $matchingFiles = Get-ChildItem -Path $SourceDirectory -Recurse -File -Filter $Filter -ErrorAction SilentlyContinue
        if (-not $matchingFiles -or $matchingFiles.Count -eq 0) {
            _Write-StandardMessage -Message "[SKIP] No files matching filter found in source. Skipping directory creation as CopyEmptyDirs is false." -Level 'INF'
            return
        }
    }
    ...
    # Create the destination directory if it doesn't exist.
    if (-not (Test-Path -Path $DestinationDirectory -PathType Container)) {
        _Write-StandardMessage -Message "[CREATE] Destination directory '$DestinationDirectory' does not exist. Creating it." -Level 'INF'
        New-Item -ItemType Directory -Path $DestinationDirectory | Out-Null
        $dirsCreated++
    }
    ...
    switch ($CleanDestination) {
        'RootFiles' {
            _Write-StandardMessage -Message "[STATUS] Cleaning destination root: removing extra files not present in source." -Level 'DBG'
            $destRootFiles = Get-ChildItem -Path $DestinationDirectory -File -Filter $Filter
            foreach ($destFile in $destRootFiles) {
                $sourceFilePath = Join-Path -Path $SourceDirectory -ChildPath $destFile.Name
                if (-not (Test-Path -Path $sourceFilePath -PathType Leaf)) {
                    _Write-StandardMessage -Message "[REMOVE] Removing file: $($destFile.FullName)" -Level 'DBG'
                    Remove-Item -Path $destFile.FullName -Force
                    $filesRemoved++
                }
            }
        ...
        'FilesRecursive' {
            _Write-StandardMessage -Message "[STATUS] Cleaning destination tree: removing extra files not present in source (recursive)." -Level 'DBG'
            $destFiles = Get-ChildItem -Path $destFullPath -Recurse -File -Filter $Filter
            foreach ($destFile in $destFiles) {
                $relative = $destFile.FullName.Substring($destFullPath.Length).TrimStart('\')
                $sourceFilePath = Join-Path -Path $sourceFullPath -ChildPath $relative
        ...
        'MirrorTree' {
            _Write-StandardMessage -Message "[STATUS] Full clean: removing extra files and directories to mirror source." -Level 'DBG'
            ...
            foreach ($destDir in $destDirs) {
                ...
                $sourceDirPath = Join-Path -Path $sourceFullPath -ChildPath $relativeDir
                if (-not (Test-Path -Path $sourceDirPath -PathType Container)) {
                    _Write-StandardMessage -Message "[REMOVE] Removing directory: $($destDir.FullName)" -Level 'DBG'
                    Remove-Item -Path $destDir.FullName -Recurse -Force
                    $dirsRemoved++
                }
            }
        ...
        default {
            _Write-StandardMessage -Message "[STATUS] CleanDestination = 'None'. No destination cleanup requested." -Level 'DBG'
        }
    }

    if ($CopyEmptyDirs) {
        _Write-StandardMessage -Message "[STATUS] Recreating complete directory structure from source." -Level 'DBG'
        Get-ChildItem -Path $sourceFullPath -Recurse -Directory | ForEach-Object {
            $relativePath = $_.FullName.Substring($sourceFullPath.Length)
            $newDestDir   = Join-Path -Path $destFullPath -ChildPath $relativePath
            if (-not (Test-Path -Path $newDestDir)) {
                _Write-StandardMessage -Message "[CREATE] Creating directory: $newDestDir" -Level 'DBG'
                New-Item -ItemType Directory -Path $newDestDir | Out-Null
                $dirsCreated++
            }
        }
        ...
        _Write-StandardMessage -Message "[STATUS] Creating directories only for files matching the filter." -Level 'DBG'
        foreach ($file in $matchingFiles) {
            $sourceDir   = Split-Path -Path $file.FullName -Parent
            $relativeDir = $sourceDir.Substring($sourceFullPath.Length)
            $newDestDir  = Join-Path -Path $destFullPath -ChildPath $relativeDir
    ...
    _Write-StandardMessage -Message "[STATUS] Copying files from source to destination." -Level 'INF'
    if ($CopyEmptyDirs) {
        $filesToCopy = Get-ChildItem -Path $SourceDirectory -Recurse -File -Filter $Filter
    }
    else {
    ...
    foreach ($file in $filesToCopy) {
        ...
        if (-not $ForceOverwrite -and (Test-Path -Path $destFile)) {
            _Write-StandardMessage -Message "[SKIP] Skipping existing file (overwrite disabled): $destFile" -Level 'DBG'
            $filesSkipped++
            continue
        }
        ...
        $destDir = Split-Path -Path $destFile -Parent
        if (-not (Test-Path -Path $destDir)) {
            _Write-StandardMessage -Message "[CREATE] Creating directory for file: $destDir" -Level 'DBG'
            New-Item -ItemType Directory -Path $destDir | Out-Null
            $dirsCreated++
        }

        _Write-StandardMessage -Message "[STATUS] Copying file: $($file.FullName) to $destFile" -Level 'DBG'
        if ($ForceOverwrite) {
            Copy-Item -Path $file.FullName -Destination $destFile -Force
        }
        else {
