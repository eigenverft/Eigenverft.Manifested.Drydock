...
function Get-CallerScriptInfo {
<#
.SYNOPSIS
Returns the current script path ($PSCommandPath) and a FileInfo object for the caller script.

.DESCRIPTION
Provides a structured view including:
- CurrentScriptPath: path of the script where this function executes.
- CallerScriptPath: path of the immediate caller script (if any).
- CallerFileInfo: System.IO.FileInfo object for the caller file, usable for "reflection".
- CallStack: raw call stack for deeper diagnostics.

.PARAMETER Depth
How far up the call stack to inspect for the caller script. 1 = immediate caller.

.EXAMPLE
Get-CallerScriptInfo
Returns the current script path, the immediate caller’s script path, and a FileInfo for that caller.

.EXAMPLE
Get-CallerScriptInfo -Depth 2
Looks two frames up the call stack to determine the caller script.

.NOTES
Works in PowerShell 5.x. In interactive sessions, caller script may be null.
#>
    [CmdletBinding()]
    param(
        [int]$Depth = 1
    ...
    # Reviewer note: $PSCommandPath may be $null in interactive use; fall back to $MyInvocation when possible.
    $currentPath = if ($PSCommandPath) { $PSCommandPath } else { $MyInvocation.MyCommand.Path }

    # Reviewer note: Get-PSCallStack gives the most reliable view of the caller script in PS5.
    $stack = Get-PSCallStack -ErrorAction SilentlyContinue

    # Try to resolve the caller script at the requested depth, if available.
    $callerPath =
        if ($stack -and $stack.Count -gt $Depth -and $stack[$Depth].ScriptName) { $stack[$Depth].ScriptName }
        elseif ($MyInvocation.ScriptName) { $MyInvocation.ScriptName }
        else { $null }

    # Safely produce a FileInfo "reflection" of the caller script.
    $callerFileInfo = if ($callerPath) { Get-Item -LiteralPath $callerPath -ErrorAction SilentlyContinue } else { $null }

    [pscustomobject]@{
        CurrentScriptPath = $currentPath
        CallerScriptPath  = $callerPath
        CallerFileInfo    = $callerFileInfo     # System.IO.FileInfo or $null
        CallStack         = $stack
    }
}