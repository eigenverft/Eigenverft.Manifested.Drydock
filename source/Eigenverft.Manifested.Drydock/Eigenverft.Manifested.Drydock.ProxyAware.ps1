...
'@
    ...
    function Test-RelayCandidatePortOpen {
        param(
            [Parameter(Mandatory = $true)]
            [uri]$ProxyUri,
    ...
    function Try-ResolveLocalRelayProxy {
        ...
        foreach ($candidate in (Get-LocalRelayProxyCandidates)) {
            if (-not (Test-RelayCandidatePortOpen -ProxyUri $candidate)) {
    ...
    function Test-PersistedProfile {
        ...
        switch ($mode) {
            ...
            'LocalRelayProxy' {
                ...
                if (-not (Test-RelayCandidatePortOpen -ProxyUri $proxyUri)) {
    ...
    try {
        ...
        # Try a local relay on loopback or the current default gateway before
        # system proxy. This is useful when users run tools like px or similar
        # local proxy helpers on the host machine.
