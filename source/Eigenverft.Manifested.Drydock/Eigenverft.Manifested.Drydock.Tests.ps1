...
#>
    ...
    function _Write-StandardMessage {
        ...
        $sev = $sevMap[$lvl]
        if ($null -eq $sev) { $lvl='INF'; $sev=$sevMap['INF'] }

        $gate = $sevMap[$min]
        if ($null -eq $gate) { $min='INF'; $gate=$sevMap['INF'] }
    ...
    if ($ThrowIfNotFound) {
        $msg = "Required command '$Command' was not found in PATH (Type=$Type)."
        _Write-StandardMessage -Message $msg -Level 'ERR'
        throw $msg
    }

    if ($ExitIfNotFound) {
        $msg = "Required command '$Command' was not found in PATH (Type=$Type). Exiting with code $ExitCode."
        _Write-StandardMessage -Message $msg -Level 'ERR'
        exit $ExitCode
    }
